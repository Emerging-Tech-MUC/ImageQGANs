...
def main(n_data_samples=10_000,
    ...
    mnist = np.load("test_mnist_raw.npy")
    mnist_labels = np.load("test_mnist_labels_raw.npy")
    ...
    n_data_samples = min(n_data_samples, len(mnist))
    ...
    save_path = (("" if n_channels == 1 else f"{n_channels}ch_") +
                 f"mnist_{digits_str}_{img_size}x{img_size}")
